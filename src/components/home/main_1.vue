<script setup lang="ts" name="main_1">
import { ref, onMounted } from 'vue';
import axios from "axios";
import Config from '@/config/Config';
import { toast, type ToastOptions } from 'vue3-toastify';

const hitokotoText = ref<string>('权酱是个大吃货');
const fromText = ref<string>('xiaoqvan');
...
const fetchHitokoto = async (): Promise<void> => {
    ...
    if (now - lastFetchTime < 2000) {
        toast(`<i class="fa-solid fa-circle-exclamation"></i> 不能点击太快了哦`, {
            autoClose: 1000,
        } as ToastOptions);
        return;
    }
    lastFetchTime = now;
    try {
        const response = await axios.get<{ hitokoto: string, from: string }>('https://v1.hitokoto.cn?max_length=19');
